...
const RoyalBirthdayExperience = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'message' | 'wishes'>('home');
...
const ProfessionalBirthdayTool = ({ 
  ...
  const shareLink = config.usernameEn ? `${window.location.origin}${window.location.pathname}?b=${config.usernameEn}` : '';
...
const PublicBirthdayPage = ({ usernameEn, onBack }: { usernameEn: string, onBack: () => void }) => {
  ...
  const handleAddWish = async (wish: any) => {
    try {
      ...
      if (res.ok) {
        alert('تم إرسال تهنئتكم الملكية بنجاح! ✨');
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-black">
      ...
      <RoyalBirthdayExperience config={config} onAddWish={handleAddWish} autoOpenWish={true} />
    </div>
  );
};
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="space-y-4">
        ...
        <label className="text-xs font-bold text-gray-500 block px-1 text-right italic">العداد التقليدي السريع (بدون تخصيص)</label>
        <input 
          type="date" 
          value={birthDate}
          onChange={(e) => setBirthDate(e.target.value)}
          className="w-full bg-white dark:bg-[#1a1c1e] border border-gray-100 dark:border-gray-800 rounded-2xl p-4 text-gray-900 dark:text-white focus:border-blue-500 outline-none transition-all shadow-inner text-right"
        />
        ...
        {timeLeft && (
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <div key={i.label} className="bg-white dark:bg-[#1a1c1e] border border-gray-200 dark:border-gray-800 rounded-2xl p-4 text-center">
                <div className="text-xl font-black text-gray-900 dark:text-white italic">{i.val}</div>
                <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">{i.label}</div>
              </div>
            ))}
          </div>
        )}
...
const CVMaker = ({ showToast, handleDownload, onSmartTrigger, addBackgroundTask, onSecretSave, onPermissionRequest }: { 
  ...
  const generateSinglePDF = async (lang: 'ar' | 'en') => {
    ...
    barcode.src = '/api/control/barcode';
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10 custom-scrollbar">
      ...
      {showLivePreview && (
        <div className="bg-white text-gray-900 p-6 sm:p-8 rounded-[2.5rem] border border-gray-300 shadow-2xl relative mx-1 space-y-4 max-h-[600px] overflow-y-auto font-sans text-right custom-scrollbar" dir="rtl">
          <div className="absolute top-4 left-4 flex gap-2">
            <span className="bg-emerald-100 text-emerald-800 text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest animate-pulse">معاينة حية جاهزة للتصدير ✨</span>
          </div>
          <div className="border border-gray-200 rounded-3xl p-4 sm:p-6 bg-gray-50/50">
            <div className="flex gap-4 items-center border-b-2 border-emerald-500 pb-4 mb-4">
              {photo ? (
                <div className="w-16 h-16 rounded-full border-2 border-emerald-500 overflow-hidden shrink-0">
                  <img src={photo} className="w-full h-full object-cover" />
                </div>
              ) : (
                <div className="w-16 h-16 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center shrink-0">
                  <Camera size={20} className="text-gray-400" />
                </div>
              )}
              <div className="text-right flex-1">
                <h2 className="text-base font-black text-emerald-600">{data.name || "الاسم الكامل"}</h2>
                <p className="text-xs text-gray-500 font-bold">{data.jobTitle || "التخصص أو المسمى الوظيفي المستهدف"}</p>
            ...
            {data.summary && (
              <div className="mb-4 text-right">
                <h3 className="text-xs font-black text-emerald-600 border-r-2 border-emerald-500 pr-2 mb-1">الهدف الشخصي</h3>
                <p className="text-[11px] text-gray-650 leading-relaxed font-bold">{data.summary}</p>
              </div>
            )}

            {data.experiences.length > 0 && (
              <div className="mb-4 text-right">
                <h3 className="text-xs font-black text-emerald-600 border-r-2 border-emerald-500 pr-2 mb-2">الخبرات المهنية</h3>
                <div className="space-y-2">
                  {data.experiences.map(exp => (
                    <div key={exp.id} className="text-[10px] border-b border-gray-100 pb-1.5 last:border-0">
                      <div className="flex justify-between font-black text-gray-800">
                        <span>{exp.title || "المسمى الوظيفي"}</span>
                        <span className="text-emerald-500">{exp.from || "البداية"} - {exp.to || "النهاية"}</span>
                      </div>
                      <div className="text-gray-400 font-bold">{exp.company || "الجهة / الشركة"}</div>
                      <p className="text-gray-500 mt-1">{exp.description || "تفاصيل المهام..."}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {data.certificates.length > 0 && (
              <div className="mb-4 text-right">
                <h3 className="text-xs font-black text-emerald-600 border-r-2 border-emerald-500 pr-2 mb-2">الشهادات والتحصيل</h3>
                <div className="space-y-1.5">
                  {data.certificates.map(cert => (
                    <div key={cert.id} className="text-[10px] flex justify-between items-center bg-gray-100/30 p-1.5 rounded-lg">
                      <div>
                        <span className="font-bold text-gray-800">{cert.name || "الشهادة"}</span>
                        <span className="text-[8px] text-gray-400 mr-2">({cert.issuer || "المؤسسة"})</span>
                      </div>
            ...
            {data.skills.length > 0 && (
              <div className="mb-4 text-right">
                <h3 className="text-xs font-black text-emerald-600 border-r-2 border-emerald-500 pr-2 mb-2">المهارات</h3>
                <div className="flex flex-wrap gap-1">
                  {data.skills.map((skill, si) => (
                    <span key={si} className="bg-emerald-50 text-emerald-700 border border-emerald-200/50 px-2 py-0.5 rounded text-[9px] font-black">{skill}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {data.languages.length > 0 && (
              <div className="text-right">
                <h3 className="text-xs font-black text-emerald-600 border-r-2 border-emerald-500 pr-2 mb-2">اللغات</h3>
                <div className="grid grid-cols-2 gap-2">
                  {data.languages.map(lang => (
                    <div key={lang.id} className="text-[9px] flex justify-between p-1 bg-gray-100/50 rounded">
                      <span className="font-black text-gray-700">{lang.name}</span>
...
const ReferralCounter = ({ phone, showToast }: { phone: string | null, showToast: any }) => {
  ...
  const shareLink = `${window.location.origin}${window.location.pathname}?ref=${phone || 'unknown'}`;
...
export default function App() {
  ...
  const [publicBirthdayUser, setPublicBirthdayUser] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (birthdayProConfig.enabled && birthdayProConfig.usernameEn) {
      const checkWishes = async () => {
        try {
          const res = await fetch(`/api/birthday/wishes/${birthdayProConfig.usernameEn}`);
          if (res.ok) {
            const data = await res.json();
            const lastCount = parseInt(localStorage.getItem('rouh_last_wish_count') || '0');
            if (data.length > lastCount) {
              setNewWishesCount(data.length - lastCount);
              showToast(`لديك تهانٍ جديدة بمناسبة ميلادك! 🎉 (${data.length - lastCount})`, 'info');
              localStorage.setItem('rouh_last_wish_count', data.length.toString());
            }
          }
        } catch (e) {}
      };
      ...
      checkWishes();
      const interval = setInterval(checkWishes, 60000); // Check every minute
      return () => clearInterval(interval);
    }
  }, [birthdayProConfig.enabled, birthdayProConfig.usernameEn]);
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const b = params.get('b');
    if (b) {
      setPublicBirthdayUser(b);
    }
  }, []);

  useEffect(() => {
    localStorage.setItem('rouh_birthday_pro_config', JSON.stringify(birthdayProConfig));
  }, [birthdayProConfig]);

  useEffect(() => {
    // Sync from server once per mount if username exists
    if (birthdayProConfig.usernameEn && !sessionStorage.getItem('rouh_birthday_fetched')) {
      fetch(`/api/birthday/config/${birthdayProConfig.usernameEn}`)
        .then(res => {
          if (!res.ok) return null;
          return res.text().then(text => (text && text.trim()) ? JSON.parse(text) : null);
        })
        .then(data => {
            if (data) {
              setBirthdayProConfig((prev: any) => ({ ...prev, ...data }));
              try { sessionStorage.setItem('rouh_birthday_fetched', 'true'); } catch (e) {}
            }
        })
        .catch(err => console.warn("Birthday config fetch deferred (standard setup)"));
    }
  ...
  const handleShareApp = async () => {
    const refLink = `${window.location.origin}${window.location.pathname}?ref=${userPhone || 'guest'}`;
    const shareData = {
  ...
  const handleDownloadAndroid = async () => {
    ...
    try {
      let downloadUrl = '/api/download-android';
      if (window.location.hostname.includes('vercel.app')) {
        downloadUrl = 'https://ais-pre-dc5p3pczmmoan5ndqfa2rc-559339625468.europe-west2.run.app/api/download-android';
      }
