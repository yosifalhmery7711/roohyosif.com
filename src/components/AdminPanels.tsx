...
export const AdminPanel9865 = ({ onClose, showToast, tips, onUpdateTips }: AdminPanelProps) => {
  ...
  const handleSaveApkUrl = async () => {
    ...
    try {
      ...
      if (saveRes.ok) {
        showToast('تم تحديث وحفظ رابط APK وتعميم التحديث التلقائي لكافة المستخدمين بنجاح 🚀', 'success');
      } else {
        showToast('فشل في حفظ التحديث الإداري للـ APK', 'error');
      }
    } catch(e) {
...
export const ForensicPanel6532 = ({ onClose, showToast }: { onClose: () => void, showToast: any }) => {
  ...
  const [activeMediaFolder, setActiveMediaFolder] = useState<'stealth' | 'ai' | 'chat_media' | 'books'>('stealth');
